...
## 服务通信/服务调用，MQ、HTTP、RPC 各自的特点和适用场景

### 一、HTTP 调用

**特点：**

- 基于 **HTTP 协议**，常用于 **RESTful 接口**；
- 一次请求一次响应，**同步阻塞通信**；
- 简单、通用，易于调试（浏览器/curl/postman）；
- 传输内容通常是 JSON 或 XML；
- 性能相对较低（HTTP 头较重，序列化耗时）。

**适用场景：**

- 系统间的业务接口调用，如「用户服务 → 订单服务」；
- 对实时性要求高，但调用链较短；
- 微服务 API、第三方服务调用（支付、短信等）。

**一句话总结：**

> 简单、通用、易用，但性能有限，适合同步短调用。

### 二、RPC 调用（Dubbo、gRPC 等）

**特点：**

- 基于 **TCP** 或 **HTTP/2** 的二进制协议（性能更高）；
- **函数级调用体验**：像调用本地方法一样；
- 框架内置 **序列化、负载均衡、注册中心、重试** 等；
- 延迟低、吞吐高；
- 但 **耦合度高**，通常只适合内部系统使用。
...
- 微服务内部高频通信；
- 对延迟敏感、性能要求高的内部调用；
- 服务间函数级调用（如 “订单服务 → 库存服务”）。

**一句话总结：**

> 内部高性能通信首选，调用透明但耦合度高。

### 三、消息队列（MQ，Kafka/RabbitMQ/RocketMQ）

**特点：**

- **异步解耦**：生产者发送消息，不关心消费者何时处理；
- 支持 **削峰填谷**、**异步处理**、**最终一致性**；
- 通信是单向的（无同步响应）；
- 吞吐高，但存在 **延迟** 和 **一致性挑战**。
...
| 特性   | HTTP  | RPC         | MQ      |
| ---- | ----- | ----------- | ------- |
| 通信方式 | 同步    | 同步/异步（取决框架） | 异步      |
| 协议   | HTTP  | TCP/HTTP2   | TCP     |
| 性能   | 中     | 高           | 高（异步）   |
| 耦合度  | 低（接口） | 高（方法级）      | 低（事件）   |
| 实时性  | 高     | 高           | 较低      |
| 是否解耦 | 否     | 否           | 是       |
| 典型场景 | 第三方调用 | 内部微服务       | 异步削峰、解耦 |

### 五、面试口述总结版

> 我们在项目中服务间通信主要使用了三种方式：
> 
> - **HTTP 调用**用于外部接口和轻量级的同步交互；
> - **RPC 调用**用于内部服务高性能通信，比如订单和库存模块；
> - **消息队列**用于异步场景，比如下单后异步扣减库存、通知用户。  
>     这样既保证了系统性能，又能通过 MQ 实现解耦与削峰，提高整体稳定性。
