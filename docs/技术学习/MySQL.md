...
语法：  
```sql
ALTER TABLE 表名 ADD [COLUMN] 字段名 数据类型 [约束条件] [FIRST | AFTER 已有字段名];
```  
示例：  
```sql
-- 给 user 表添加 age 字段（INT 类型，非空，默认 0），放在 name 字段后面
ALTER TABLE user ADD COLUMN age INT NOT NULL DEFAULT 0 AFTER name;

-- 添加一个新字段到表的第一列
ALTER TABLE user ADD COLUMN id_card VARCHAR(20) FIRST;
```  

### 2. 修改字段（`MODIFY` / `CHANGE`）
- **`MODIFY`**：仅修改字段的数据类型、约束或位置，不改变字段名。  
- **`CHANGE`**：可以同时修改字段名和字段定义（更灵活）。  

语法：  
```sql
-- MODIFY 用法
ALTER TABLE 表名 MODIFY 字段名 新数据类型 [新约束] [新位置];

-- CHANGE 用法（旧字段名 新字段名 新定义）
ALTER TABLE 表名 CHANGE 旧字段名 新字段名 新数据类型 [新约束] [新位置];
```  
示例：  
```sql
-- 将 user 表的 age 字段从 INT 改为 TINYINT，允许为 NULL
ALTER TABLE user MODIFY age TINYINT NULL;

-- 将字段名 age 改为 user_age，类型改为 SMALLINT，默认 18
ALTER TABLE user CHANGE age user_age SMALLINT NOT NULL DEFAULT 18;
```  
...
示例：  
```sql
-- 删除 user 表的 id_card 字段
ALTER TABLE user DROP COLUMN id_card;
```  
...
-- 唯一索引（字段值不可重复）
ALTER TABLE 表名 ADD UNIQUE [INDEX | KEY] 索引名 (字段名);
...
示例：  
```sql
-- 给 t_user 表的 name 字段添加普通索引
ALTER TABLE t_user ADD INDEX idx_name (name);

-- 给 email 字段添加唯一索引（确保邮箱不重复）
ALTER TABLE t_user ADD UNIQUE KEY uk_email (email);
```  

### 6. 删除索引（`DROP INDEX` / `DROP PRIMARY KEY`）
语法：  
```sql
-- 删除普通索引/唯一索引（通过索引名）
ALTER TABLE 表名 DROP INDEX 索引名;

-- 删除主键索引（无需指定名称，因为只有一个）
...
示例：  
```sql
-- 将 t_user 表的 user_age 字段默认值改为 20
ALTER TABLE t_user ALTER COLUMN user_age SET DEFAULT 20;
```  

### 8. 添加外键约束（`ADD FOREIGN KEY`）
语法：  
```sql
ALTER TABLE 子表名 
ADD CONSTRAINT 外键名 
...
示例：  
```sql
-- 给 order 表的 user_id 字段添加外键，关联 t_user 表的 id
ALTER TABLE `order` 
ADD CONSTRAINT fk_order_user 
FOREIGN KEY (user_id) 
REFERENCES t_user (id) 
ON DELETE CASCADE; -- 父表记录删除时，子表关联记录也删除
```  

### 注意事项
1. **锁表风险**：`ALTER TABLE` 在执行时可能会锁表（尤其是 InnoDB 引擎在修改表结构时），对于大表可能导致长时间阻塞，建议在低峰期执行。  
2. **数据安全**：修改字段类型或删除字段前，需备份数据，避免数据丢失（如 `INT` 转 `VARCHAR` 通常安全，但 `VARCHAR(20)` 转 `VARCHAR(10)` 可能截断数据）。  
3. **兼容性**：不同 MySQL 版本对 `ALTER TABLE` 的支持略有差异（如 8.0 支持更多在线 DDL 操作），需注意版本特性。  

通过 `ALTER TABLE`，可以灵活调整表结构以适应业务变化，但需谨慎操作以避免影响线上服务。

## 通过系统信息表，查询一个字段是否在其他表中作为外键约束

要查询 MySQL 中 A 表的 `col` 列是否被其他表作为外键关联，可以通过查询系统信息表 `INFORMATION_SCHEMA.KEY_COLUMN_USAGE` 实现。该表记录了所有表的键约束信息（包括外键），通过筛选条件可定位关联关系。
...
WHERE 
  REFERENCED_TABLE_NAME = 'A'  -- 被引用的表名（A表）
  AND REFERENCED_COLUMN_NAME = 'col'  -- 被引用的列名（A表的col列）
  AND TABLE_SCHEMA = 'your_database_name';  -- 数据库名（替换为实际库名）
```

### 语句说明
- **`INFORMATION_SCHEMA.KEY_COLUMN_USAGE`**：MySQL 系统表，存储所有表的键约束（主键、外键、唯一键等）信息。  
- **筛选条件**：  
  - `REFERENCED_TABLE_NAME = 'A'`：指定被引用的表是 `A` 表。  
  - `REFERENCED_COLUMN_NAME = 'col'`：指定被引用的列是 `A` 表的 `col` 列。  
  - `TABLE_SCHEMA = 'your_database_name'`：限定当前数据库（避免跨库查询，需替换为实际数据库名）。  

### 结果解读
- 若查询返回结果，说明 `A.col` 被其他表作为外键关联，结果中包含：  
  - 外键约束的名称（`CONSTRAINT_NAME`）；  
  - 引用该外键的表名（`TABLE_NAME`，即关联表）；  
  - 关联表中作为外键的列名（`COLUMN_NAME`）。  
- 若查询无结果，说明 `A.col` 没有被任何表作为外键关联。  

### 示例
假设数据库 `test` 中，`A` 表的 `id` 列被 `B` 表的 `a_id` 列作为外键关联，执行：  
```sql
SELECT 
...
表示 `B` 表的 `a_id` 列通过外键 `fk_b_a` 关联了 `A` 表的 `id` 列。

通过该方法可快速定位某列的外键引用关系，避免修改或删除字段时因外键约束导致失败。
...
![](assets/MySQL/三分恶面渣逆袭：update%20执行.png)

1. 拿到 update 语句，记录 undo log 和 redo log，用于事务回滚和数据持久化，标记 redo log 的记录为 prepare。
2. MySQL 根据 update 语句获取行锁，更新数据，将数据页标记为脏页，此时数据仍在 buffer pool 中，还未写入到磁盘。后台线程会在合适时机，批量将脏页刷入磁盘。
3. 提交事务，标记 redo log 的记录为 committed，释放行锁。
4. 如果 MySQL 开启了 binlog，还会将更新操作记录到 binlog 中，主要用于主从复制以及数据恢复，可以结合 redo log 进行点对点的恢复。binlog 的写入通常发生在事务提交时，与 redo log 共同构成“两阶段提交”，确保两者的一致性。
...
4. **二进制日志 binlog**：用于主从复制和数据恢复，记录所有修改数据库状态的 SQL 语句，以及每个语句的执行时间，如 INSERT、UPDATE、DELETE 等，但不包括 SELECT 和 SHOW 这类的操作。
5. **重做日志 redo log**：用于崩溃恢复，保证事务持久性，记录对于 InnoDB 表的每个写操作，不是 SQL 级别的，而是物理级别的，主要用于崩溃恢复。
...
![](assets/MySQL/greatsql%20社区：Redo%20Log的刷盘策略.png)
...
+ **问题**：binlog 属于 Server 层，与存储引擎无关，无法直接操作物理数据页。所以还是需要 redo log 和 undo log 来实现 ACID。
...
MySQL 中 Redo Log 的“两阶段提交”（prepare + commit）是为了保证 **Redo Log 与 Binlog 的数据一致性**，这是分布式系统中“原子提交”思想的典型应用，核心解决“单机事务日志与主从复制日志如何同步”的问题。**

![](assets/MySQL/阿里：MySQL%20两阶段提交.png)
...
### 为什么需要“两阶段提交”？
假设没有两阶段提交，事务提交时可能先写 Redo Log 再写 Binlog，或先写 Binlog 再写 Redo Log，两种情况都可能导致日志不一致：  
- **先写 Redo Log 后崩溃**：Redo Log 已记录事务，但 Binlog 未记录——主库恢复后事务存在，从库因 Binlog 缺失导致数据不一致。  
- **先写 Binlog 后崩溃**：Binlog 已记录事务，但 Redo Log 未记录——主库恢复后事务丢失，从库却已同步该事务，数据不一致。  

两阶段提交通过“分阶段标记日志状态”，确保两种日志要么“都成功记录”，要么“都不生效”，避免中间状态的不一致。

### 两阶段提交的详细流程
以一个更新事务为例，步骤如下：  
1. **事务执行阶段**：修改 Buffer Pool 中的数据页（标记为脏页），同时生成 Redo Log 日志（记录数据页的物理修改），但暂不写入磁盘。  
2. **Prepare 阶段**：  
   - 将 Redo Log 写入磁盘（持久化），并标记 Redo Log 的状态为 **prepare**（表示“准备提交”）。  
   - 此时事务尚未真正提交，但 Redo Log 已确保“即使崩溃，事务数据也可恢复”。  
3. **写入 Binlog**：  
   - 将事务的逻辑操作（如 `UPDATE t SET a=1 WHERE id=1`）写入 Binlog 并持久化到磁盘。  
4. **Commit 阶段**：  
   - 再次写入 Redo Log，将其状态从 **prepare** 改为 **commit**（表示“最终提交”）。  
   - 此时事务正式完成，Redo Log 和 Binlog 均成功记录，状态一致。  

### 崩溃恢复时的逻辑
若在两阶段中间崩溃（如 Prepare 后、Commit 前），MySQL 重启时会：  
1. 扫描 Redo Log，找到所有状态为 **prepare** 的事务。  
2. 检查这些事务的 Binlog 是否完整（是否已成功写入）：  
   - 若 Binlog 完整：将 Redo Log 状态改为 **commit**，事务生效（保证与 Binlog 一致）。  
   - 若 Binlog 不完整：回滚该事务（放弃 Redo Log 中的修改，保证与 Binlog 一致）。  

### 核心目的
两阶段提交通过“Redo Log 先 prepare 再 commit”的状态标记，将 Redo Log 和 Binlog 的写入“绑定为一个原子操作”，确保：  
- 主库崩溃恢复后，数据状态与 Binlog 完全一致；  
- 从库通过 Binlog 同步的数据，与主库最终状态一致。  

这是 MySQL 保证“事务 ACID 特性”和“主从数据一致性”的关键设计。
